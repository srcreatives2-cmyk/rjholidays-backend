...
```js
node -e "const crypto=require('crypto'); const p='YOUR_CHOSEN_PASSWORD'; const salt=crypto.randomBytes(16).toString('hex'); const key=crypto.scryptSync(p,salt,64).toString('hex'); console.log(`${salt}:${key}`)"
```

Copy the output — this goes into `ADMIN_PASSWORD_HASH`. The backend uses Node.js `crypto.scryptSync`, not bcrypt.
